...
async function maybePreview(
  entries: BqbEntry[],
  opts: { preview?: boolean; renderer?: PreviewRenderer }
) {
  if (!opts.preview || entries.length === 0) return;
...
program
  ...
  .action(
    async (
      query: string,
      opts: { limit: string; category?: string; renderer?: PreviewRenderer }
    ) => {
      try {
        const entries = await loadData();
        const results = search(entries, query, {
        ...
        const renderer = await previewEntries(
          results.map((r) => r.entry),
          {
            renderer: opts.renderer ?? "auto",
          }
