...
function detectRenderer(): Exclude<PreviewRenderer, "auto"> {
...
async function ensurePreviewFile(entry: BqbEntry) {
...
function getChafaArgs(filePath: string, renderer: Exclude<PreviewRenderer, "auto" | "open">) {
...
export async function previewEntries(
  ...
  if (entries.length === 0) return "symbols" as const;

  const requestedRenderer = options.renderer ?? "auto";
  const renderer =
    requestedRenderer === "auto" ? detectRenderer() : requestedRenderer;
  ...
  if (renderer === "open") {
    ...
    throw new Error(
      "未找到 chafa。请先安装 chafa，或使用 --renderer open 走系统图片预览。"
    );
  }
